...
function SettingsHub:Build(_, parent)
    ...
    local title = UI.Text(parent, "GameFontNormalHuge", "НАСТРОЙКИ", C.accent)
    title:SetPoint("TOPLEFT", 22, -20)
    local note = UI.Text(parent, "GameFontHighlightSmall", "Единый центр функций вместо набора мелких аддонов", C.muted)
    ...
    local sideTitle = UI.Text(sidebar, "GameFontNormalSmall", "РАЗДЕЛЫ", C.muted)
    ...
    local automationPage = MakePage("automation", "АВТОМАТИЗАЦИЯ",
        "Задания, приглашения и действия у торговцев")
    local groupPage = MakePage("groups", "ГРУППЫ И КЛЮЧИ",
        "Приглашения, эпохальные ключи и умный клик")
    local interfacePage = MakePage("interface", "ИНТЕРФЕЙС",
        "Внешний вид, фреймы и расположение элементов")
    local lootPage = MakePage("loot", "ДОБЫЧА",
        "Окно добычи, голосования и история предметов")
    local systemPage = MakePage("system", "СИСТЕМА",
        "Перехват ошибок и диагностический журнал")
    ...
    Heading(automationPage, "ЗАДАНИЯ И ПРИГЛАШЕНИЯ", 28, -84, 764)
    self:AddCheck(automationPage, "autoQuests", "Автоматически принимать и сдавать задания", 28, -118)
    self:AddCheck(automationPage, "summon", "Автоматически принимать призыв вне боя", 28, -152)
    self:AddCheck(automationPage, "resurrection", "Автоматически принимать воскрешение", 28, -186)
    self:AddCheck(automationPage, "resNoCombat", "Не принимать боевое воскрешение", 52, -220)

    Heading(automationPage, "ТОРГОВЦЫ", 28, -270, 764)
    self:AddCheck(automationPage, "sellJunk", "Автоматически продавать серые предметы", 28, -304)
    self:AddCheck(automationPage, "repair", "Автоматически ремонтировать экипировку", 28, -338)
    self:AddCheck(automationPage, "guildRepair", "Сначала использовать средства гильдии", 52, -372)
    ...
    self:AddCheck(interfacePage, "minimalUI", "Минимальный интерфейс MythicBoost", 28, -118, function(value)
    ...
    local castBar = UI.CheckBox(interfacePage, "Кастбар игрока в стиле Quartz",
    ...
    Heading(lootPage, "ОКНО И ПОЛУЧЕНИЕ", 28, -84, 764)
    local lootUI = UI.CheckBox(lootPage, "Встроенное компактное окно добычи",
    ...
    lootUI:HookScript("OnEnter", function(self)
        UI.Tooltip(self, "Окно добычи MythicBoost",
            "Компактные строки, голосование Need/Greed, история победителей, подсказки и сбор кликом.",
            "Внешние XLoot_Frame, XLoot_Group и XLoot_Monitor необходимо отключить.")
    end)
    lootUI:HookScript("OnLeave", GameTooltip_Hide)
    self.checks.lootUI = lootUI

    local lootAtCursor = UI.CheckBox(lootPage, "Открывать добычу рядом с курсором",
    ...
    local lootRolls = UI.CheckBox(lootPage, "Голосование за групповую добычу",
    ...
    local lootHistory = UI.CheckBox(lootPage, "Показывать, кто что получил",
    ...
    interfaceMove:HookScript("OnEnter", function(self)
        UI.Tooltip(self, "Единый режим перемещения",
            "Разблокирует фреймы, кастбар, окно ключа и добычу, если она не привязана к курсору.",
            "После расстановки отключи режим здесь же.")
    end)
    ...
    Heading(groupPage, "УМНЫЙ КЛИК", 28, -206, 764)
    local smartBuff = UI.CheckBox(groupPage, "Умный клик: давать недостающий бафф",
    ...
    smartBuff:HookScript("OnEnter", function(self)
        UI.Tooltip(self, "Умный клик: бафф",
            "Добавляет в макрос MBSmartClick групповой бафф твоего класса.",
            "Аддон ведёт макрос MBSmartClick — привяжи его один раз в клик-касте DandersFrames.",
            JP.SmartClick and JP.SmartClick:Describe() or "")
    end)
    smartBuff:HookScript("OnLeave", GameTooltip_Hide)
    self.checks.smartBuff = smartBuff

    local smartRes = UI.CheckBox(groupPage, "Умный клик: поднимать мёртвого",
    ...
    smartRes:HookScript("OnEnter", function(self)
        UI.Tooltip(self, "Умный клик: воскрешение",
            "В бою — боевое воскрешение, вне боя — обычное. Заклинание выбирает игра в момент клика.",
            "Аддон ведёт макрос MBSmartClick — привяжи его один раз в клик-касте DandersFrames.",
            JP.SmartClick and JP.SmartClick:Describe() or "")
    end)
    ...
    Heading(lootPage, "ГРУППОВАЯ ДОБЫЧА", 28, -270, 764)
    local rcLoot = UI.CheckBox(lootPage, "Отвечать на розыгрыши RCLootCouncil",
    ...
    rcLoot:HookScript("OnEnter", function(self)
        UI.Tooltip(self, "Мост к RCLootCouncil",
            "Лидер разыгрывает вещь — окно открывается у нас, ответ уходит в его RCLootCouncil.",
            "Сам RCLootCouncil тебе больше не нужен: если он стоит, отключи его, иначе ответите оба.",
            "Сверху к каждой вещи добавлен наш вердикт: BIS для спека или нет.")
    end)
    ...
    Heading(systemPage, "ПЕРЕХВАТ ОШИБОК", 28, -84, 764)
    local preventErrors = UI.CheckBox(systemPage, "Prevent Error in Game",
    ...
    preventErrors:HookScript("OnEnter", function(self)
        UI.Tooltip(self, "Prevent Error in Game",
            "Ошибки Lua перехватываются и не показываются на экране: ни окна, ни звука.",
            "Они не теряются — копятся в журнале со счётчиком повторов и переживают /reload.",
            "Отключи BugGrabber и BugSack: обработчик ошибок в игре один, и достаётся он тому, кто загрузился последним.")
    end)
    preventErrors:HookScript("OnLeave", GameTooltip_Hide)
    self.checks.preventErrors = preventErrors

    local errorLog = UI.Button(systemPage, "Открыть журнал ошибок", 210, 28)
    ...
    errorLog:HookScript("OnEnter", function(self)
        ...
        UI.Tooltip(self, "Журнал ошибок",
            ("Сейчас в журнале: %d уникальных, %d срабатываний."):format(unique, total),
            "Щелчок по строке открывает полный трейс — его можно выделить и скопировать.")
    end)
    errorLog:HookScript("OnLeave", GameTooltip_Hide)

    local automationInfo = UI.Text(automationPage, "GameFontHighlightSmall",
        "Shift временно отключает автоматизацию заданий, продажу и ремонт.", C.muted)
    ...
    local systemInfo = UI.Text(systemPage, "GameFontHighlightSmall",
        "Настройки применяются сразу. Защищённые действия во время боя безопасно пропускаются.", C.muted)
